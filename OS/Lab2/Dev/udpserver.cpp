...
static int send_next_ten(SOCKET sock, const struct sockaddr_in* addr, PendingMessage* arr, int total)
{
    int i;
    int cnt;
...
static int drain_acks(SOCKET sock, WSAEVENT ev, PendingMessage* arr, int total)
{
    char ackbuf[65535];
    ...
    for (;;)
    {
        ...
        wait_res = WSAWaitForMultipleEvents(1, &ev, FALSE, 100, FALSE);
        ...
        if (WSAEnumNetworkEvents(sock, ev, &ne) == SOCKET_ERROR)
        {
...
int main(int argc, char* argv[])
{
    ...
    for (;;)
    {
        ...
        if (drain_acks(sock, ev, msgs, total) != 0)
        {
            goto cleanup;
        }
    }
