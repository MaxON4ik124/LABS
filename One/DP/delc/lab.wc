...
double factorial(double n) {
    ...
    int intN = (int)n;
    if (intN != n) {
        printf("Warning: factorial is only calculated for integers\n");
    }
...
double applyFunction(const char* func, double value) {
    ...
    if (strcmp(func, "sin") == 0) {
        ...
        if (fabs(fmod(normalizedValue - PI2, PI)) < epsilon) {
            return (fmod(round(normalizedValue  (PI2)), 4) == 1) ? 1.0 : -1.0;
        }
        ...
        if (fabs(fmod(normalizedValue - PI2, PI)) < epsilon) {
            printf("Error: tangent is undefined at x = pi/2 + n*pi\n");
            exit(1);
        }
        result = tan(value);
        ...
        if (fabs(fmod(normalizedValue, PI)) < epsilon) {
            printf("Error: cotangent is undefined at x = n*pi\n");
            exit(1);
        }
