...
kubectl config use-context "%PROFILE%" || goto :error
minikube status -p "%PROFILE%" || goto :error

echo ==================================================
echo Creating the namespace, storage, and node labels
echo ==================================================

kubectl apply -f "%PROJECT_DIR%namespace.yaml" || goto :error
minikube -p "%PROFILE%" addons enable csi-hostpath-driver || goto :error

kubectl label node "%PROFILE%-m02" app-variant=v1 --overwrite || goto :error
kubectl label node "%PROFILE%-m03" app-variant=v2 --overwrite || goto :error
kubectl label node "%PROFILE%-m04" app-variant=v3 --overwrite || goto :error
...
kubectl apply -f "%PROJECT_DIR%db-secret.yaml" || goto :error
kubectl apply -f "%PROJECT_DIR%orders-db.yaml" || goto :error
kubectl apply -f "%PROJECT_DIR%db.yaml" || goto :error

kubectl rollout status statefulset/shop-db -n "%NAMESPACE%" --timeout=300s
...
docker build -f Dockerfile --build-arg APP_VARIANT=v1 -t xxe-demo:v1 . || goto :error
docker build -f Dockerfile --build-arg APP_VARIANT=v2 -t xxe-demo:v2 . || goto :error
docker build -f Dockerfile --build-arg APP_VARIANT=v3 -t xxe-demo:v3 . || goto :error

minikube -p "%PROFILE%" image load xxe-demo:v1 --overwrite || goto :error
minikube -p "%PROFILE%" image load xxe-demo:v2 --overwrite || goto :error
minikube -p "%PROFILE%" image load xxe-demo:v3 --overwrite || goto :error

echo ==================================================
echo Deploying the applications
echo ==================================================

kubectl apply -f "%PROJECT_DIR%apps.yaml" || goto :error
...
docker build -f Dockerfile.proxy -t xxe-node-proxy:local . || goto :error
...
docker run -d --name xxe-v1-proxy --restart unless-stopped --network "%MINIKUBE_NETWORK%" -p 5001:5001 -e LISTEN_PORT=5001 -e TARGET_HOST=%PROFILE%-m02 -e TARGET_PORT=5001 xxe-node-proxy:local || goto :error
docker run -d --name xxe-v2-proxy --restart unless-stopped --network "%MINIKUBE_NETWORK%" -p 5002:5002 -e LISTEN_PORT=5002 -e TARGET_HOST=%PROFILE%-m03 -e TARGET_PORT=5002 xxe-node-proxy:local || goto :error
docker run -d --name xxe-v3-proxy --restart unless-stopped --network "%MINIKUBE_NETWORK%" -p 5003:5003 -e LISTEN_PORT=5003 -e TARGET_HOST=%PROFILE%-m04 -e TARGET_PORT=5003 xxe-node-proxy:local || goto :error
